...
namespace AddictedToVintage\AdminBundle\UploadHandler;
...
class UploadHandler {
    ...
    function __construct($options = null) {
        $this->options = array(
            ...
            'upload_dir' => dirname($_SERVER['SCRIPT_FILENAME']) . '/images/products/',
            'upload_url' => $this->getFullUrl() . '/images/products/',
            'param_name' => 'files',
            ...
            'image_versions' => array(
                 ...
                 */
                'thumbnail' => array(
                    'upload_dir' => dirname($_SERVER['SCRIPT_FILENAME']) . '/images/products/thumbs/',
                    'upload_url' => $this->getFullUrl() . '/images/products/thumbs/',
                    'max_width' => null,
                    'max_height' => 150
                )
            )
...
//        }
    ...
    public function create_scaled_image($file_name, $options) {
        ...
        $source_image = $this->options['upload_dir'] . $file_name;
        ...
        if (!in_array($image_type, $supported_types)) {
            ...
            switch ($image_type) {
                case 1:
                    $success = imagegif($im, $new_file_path);
                    $thumbnail = $filename . '.gif';
                    break;

                case 2:
                    $success = $thumbnail = $filename . '.jpg';
                    imagejpeg($thumb, $new_file_path, $quality);
                    break;

                case 3:
                    $success = imagepng($im, $new_file_path);
                    $thumbnail = $filename . '.png';
                    break;

                case 7:
                    $success = imagewbmp($im, $new_file_path);
