...
class CartController extends Controller {
    ...
    public function MSPPaymentPageAction() {
        ...
        return $this->render('BKBundle:MSP:betalen.html.twig', array('test' => $test));
    }
    ...
    public function shippingAction() {
        ...
        return $this->render('BKBundle:Checkout:shipping.html.twig', array('cart' => $cart, 'next_delivery_date' => $next_delivery_date, 'client' => $client, 'shippings' => $shippings, 'next_delivery_date_range' => $next_delivery_date_range));
    }
    ...
    public function paymentAction() {
        ...
        return $this->render('BKBundle:Checkout:payment.html.twig', array('cart' => $cart, 'paymethods' => $paymethods, 'discount' => $discount));
    }
    ...
    public function overviewAction() {
        ...
        return $this->render('BKBundle:Checkout:overview.html.twig', array('cart' => $cart, 'next_delivery_date' => $next_delivery_date, 'client' => $client, 'discount' => $discount, 'paymethod' => $paymethod));
    }
    ...
    public function checkoutAction() {
        ...
        $msp->merchant['notification_url'] = ltrim(BASE_URL_BK, '/') . $this->generateUrl('_notify') . '?type=initial';
        $msp->merchant['cancel_url'] = ltrim(BASE_URL_BK, '/') . $this->generateUrl('_checkout_cancel');

        // optional automatic redirect back to the shop:
        $msp->merchant['redirect_url'] = ltrim(BASE_URL_BK, '/') . $this->generateUrl('_checkout_complete');
        ...
        if ($this->getRequest()->getMethod() === 'POST') {
            ...
            $message = \Swift_Message::newInstance()
                    ...
                    ->addPart($this->renderView('BKBundle:Email:order.txt.twig', array('order' => $order, 'products' => $products, 'client' => $client)))
                    ->setBody($this->renderView('BKBundle:Email:order.html.twig', array('order' => $order, 'products' => $products, 'client' => $client)), 'text/html')
            ;
            ...
            $client = $this->getDoctrine()->getRepository('BKBundle:Client')->find($client);
            
            $email = new \Ecommerce\BKBundle\Entity\Email();
            $email->setClient($client);
            $email->setContent($this->renderView('BKBundle:Email:order.html.twig', array('order' => $order, 'products' => $products, 'client' => $client)));
        ...
        return $this->render('BKBundle:Checkout:index.html.twig', array('cart' => $cart));
    }
    ...
    public function checkoutCompleteAction() {
        ...
        return $this->render('BKBundle:Checkout:success.html.twig', array('order' => $order));
    }
    ...
    public function checkoutCancelAction() {
        ...
        return $this->render('BKBundle:Checkout:cancel.html.twig', array('order' => $order));
    }
	...
	if($this->get('kernel')->getEnvironment() == 'dev') { 
        ...
        $order = $this->getDoctrine()->getRepository('BKBundle:Orders')->findOneBy(array('ordernr' => $transactionid));
