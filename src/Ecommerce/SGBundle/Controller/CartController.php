...
use Ecommerce\SGBundle\MultiSafePay\MultiSafePay;
use Symfony\Component\Security\Core\Authentication\Token\UsernamePasswordToken;

define('BASE_URL_SG', 'http://www.addictedtovintage.nl');
...
class CartController extends Controller {
    ...
    public function MSPPaymentPageAction() {
        ...
        return $this->render('SGBundle:MSP:betalen.html.twig', array('test' => $test));
    }
    ...
    public function indexAction() {
        ...
        $shippings = $this->getDoctrine()->getRepository('SGBundle:Shipping')->findBy(array('shippingType' => $shippingType, 'active' => 1));
        
        return $this->render('SGBundle:Cart:index.html.twig', array('cart' => $cart, 'discount' => $discount, 'shippings' => $shippings, 'coupon' => $coupon));
    }
    ...
    public function shippingAction() {
        ...
        /* Fetch active shipping methods */
        $shippings = $this->getDoctrine()->getRepository('SGBundle:Shipping')->findBy(array('shippingType' => $cart->getShipping()->getShippingType(), 'active' => 1));

        $next_delivery_date = $this->getNextDeliveryDate();
        
        $next_delivery_date_range = $this->getNextDeliveryDateRange();
        
        return $this->render('SGBundle:Checkout:shipping.html.twig', array('cart' => $cart, 'next_delivery_date' => $next_delivery_date, 'client' => $client, 'shippings' => $shippings, 'next_delivery_date_range' => $next_delivery_date_range));
    }
    ...
    public function paymentAction() {
        ...
        $paymethods = $this->getDoctrine()->getRepository('SGBundle:Paymethod')->findBy(array('isActive' => 1));
        ...
        return $this->render('SGBundle:Checkout:payment.html.twig', array('cart' => $cart, 'paymethods' => $paymethods, 'discount' => $discount));
    }
    ...
    public function overviewAction() {
        ...
        return $this->render('SGBundle:Checkout:overview.html.twig', array('cart' => $cart, 'next_delivery_date' => $next_delivery_date, 'client' => $client, 'discount' => $discount, 'paymethod' => $paymethod));
    }
    ...
    public function checkoutAction() {
        ...
        $msp->merchant['notification_url'] = ltrim(BASE_URL_SG, '/') . $this->generateUrl('_notify') . '?type=initial';
        $msp->merchant['cancel_url'] = ltrim(BASE_URL_SG, '/') . $this->generateUrl('_checkout_cancel');

        // optional automatic redirect back to the shop:
        $msp->merchant['redirect_url'] = ltrim(BASE_URL_SG, '/') . $this->generateUrl('_checkout_complete');
        ...
        if ($this->getRequest()->getMethod() === 'POST') {
            ...
            $message = \Swift_Message::newInstance()
                    ...
                    ->addPart($this->renderView('SGBundle:Email:order.txt.twig', array('order' => $order, 'products' => $products, 'client' => $client)))
                    ->setBody($this->renderView('SGBundle:Email:order.html.twig', array('order' => $order, 'products' => $products, 'client' => $client)), 'text/html')
            ;
            ...
            $client = $this->getDoctrine()->getRepository('SGBundle:Client')->find($client);
            
            $email = new \Ecommerce\SGBundle\Entity\Email();
            $email->setClient($client);
            $email->setContent($this->renderView('SGBundle:Email:order.html.twig', array('order' => $order, 'products' => $products, 'client' => $client)));
        ...
        return $this->render('SGBundle:Checkout:index.html.twig', array('cart' => $cart));
    }

    public function checkoutCompleteAction() {

        $order = $this->getDoctrine()->getRepository('SGBundle:Orders')->findOneBy(array('ordernr' => $this->getRequest()->get('transactionid')));
        ...
        return $this->render('SGBundle:Checkout:success.html.twig', array('order' => $order));
    }
    ...
    public function checkoutCancelAction() {
        ...
        return $this->render('SGBundle:Checkout:cancel.html.twig', array('order' => $order));
    }
	...
	if($this->get('kernel')->getEnvironment() == 'dev') { 
        ...
        $order = $this->getDoctrine()->getRepository('SGBundle:Orders')->findOneBy(array('ordernr' => $transactionid));
