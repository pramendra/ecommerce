...
namespace Ecommerce\SGBundle\ShopManager;
...
use Ecommerce\SGBundle\Entity\Cart;
use Ecommerce\SGBundle\Entity\CartProducts;
use Ecommerce\SGBundle\Entity\Client;
use Ecommerce\SGBundle\Entity\Product;
use Ecommerce\SGBundle\Entity\Shipping;
use Ecommerce\SGBundle\Entity\ShippingTypes;
use Ecommerce\SGBundle\Entity\Orders;
use Ecommerce\SGBundle\Entity\OrdersProduct;
use Ecommerce\SGBundle\Entity\Coupon;
use Ecommerce\SGBundle\Entity\Paymethod;
...
class ShopManager extends ContainerAware {
    ...
    function initialize() {
        ...
        if ($this->session->get('cart') !== null) {
            ...
            if($cart->getId() !== null) {
                $this->cart = $this->getDoctrine()->getRepository('SGBundle:Cart')->find($cart->getId());
            } else { 
                $this->cart = new Cart();
        ...
        if ($this->session->get('order') !== null) {
            ...
            $this->order = $this->getDoctrine()->getRepository('SGBundle:Orders')->find($order->getId());
        } else {
            $this->order = null;
        ...
        if ($this->session->get('shipping') !== null) {
            ...
            $this->shipping = $this->getDoctrine()->getRepository('SGBundle:Shipping')->find(1);
    ...
    public function setShipping($shipping) {
        ...
        $this->shipping = $this->getDoctrine()->getRepository('SGBundle:Shipping')->find($shipping);
        $this->session->set('shipping', $this->shipping);        
    ...
    public function getShoppingList() {
        ...
        foreach ($list as $item) {
            ...
            if (!empty($item)) {
                ...
                $product = $this->getDoctrine()->getRepository('SGBundle:Product')->find($item);
    ...
    public function getCoupon() {
        ...
        if ($this->coupon->getId() !== null) {
            return $this->getDoctrine()->getRepository('SGBundle:Coupon')->find($this->coupon->getId());
        } else { 
            return null;
        }
    ...
    public function removeProductFromCart(Product $product) { 
        ...
        $cartProduct = $this->getDoctrine()
                ->getRepository('SGBundle:CartProducts')
        ...
        if ($containsPackage) {
            ...
            $shipping = $this->getDoctrine()
                            ->getRepository('SGBundle:Shipping')->findOneBy(array('shippingType' => 1, 'active' => 1));
            
        } else {
            /* Fetch active shipping methods */
            $shipping = $this->getDoctrine()
                            ->getRepository('SGBundle:Shipping')->findOneBy(array('shippingType' => 2, 'active' => 1));
        }
    ...
    public function addCouponCode($code) {
        ...
        $this->coupon = $this->getDoctrine()->getRepository('SGBundle:Coupon')->findOneBy(array('code' => $code));
    ...
    public function setPaymethod($request) { 
        ...
        $paymethod = $this->getDoctrine()->getRepository('SGBundle:Paymethod')->findOneBy(array('code' => $payment[0]));
    ...
    public function createOrder($request) {
        ...
        // get entity manager
        $em = $this->getDoctrine()->getEntityManager();
        $this->client = $this->getDoctrine()->getRepository('SGBundle:Client')->find($this->client->getId());
        $this->shipping = $this->getDoctrine()->getRepository('SGBundle:Shipping')->find($this->shipping->getId());
        $this->paymethod = $this->getDoctrine()->getRepository('SGBundle:Paymethod')->find($this->paymethod->getId());
        ...
        if ($this->order == null) {
            ...
            if($this->coupon->getId() != null) { 
                ...
                $coupon = $this->getDoctrine()->getRepository('SGBundle:Coupon')->find($this->coupon->getId());
            ...
            // Fetch new orderNr
            $orderNr = $this->getDoctrine()->getRepository('SGBundle:Orders')->getNewOrderNr();
    ...
    private function createCart() {
        ...
        $this->shipping = $this->getDoctrine()
                ->getRepository('SGBundle:Shipping')
    ...
    private function updateCart() {
        ...
        $cart = $this->getDoctrine()->getRepository('SGBundle:Cart')->findOneBy(array('id' => $this->cart->getId()));
